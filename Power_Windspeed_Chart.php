...
$Mysql_Query="select HOUR(TIME_S) as Hour ,MAX(GREATEST(Active_Total_Gen_Import,0)) as GEN_Max ,MIN(GREATEST(Active_Total_Gen_Import,0)) as GEN_Min ,ROUND(AVG(GREATEST(POWER,0)),2) as Power, ROUND(AVG(WINDSPEED),2) as WindSpeed, Date_S,Time_S  from $Cook_Variable[7].device_data_f7 where IMEI = '".$IMEI."' and Date_S='$Extract_Date' and Active_Total_Gen_Import between 0 and 20000000  group by HOUR(Time_S)";
...
button {
    background: linear-gradient(135deg, #1976d2, #0f4da2);
    border: none;
    color: white;
    padding: 10px 18px;
    border-radius: 8px;
...
#chart_wind, #chart_kwh, #chart_div {
    background: #fff;
    border-radius: 14px;
    ...
    width: 100%;
    max-width: 1500px;   /* Makes chart very wide on desktop */
}

/* Force Google chart SVG to stretch */
svg {
    width: 100% !important;
}
...
.graph-border {
    ...
    padding: 10px;            /* spacing so graph doesn't touch border */
    margin-top: 15px;
    border-radius: 6px;       /* optional: smooth corners */
    background-color: #fff;   /* ensures graph visibility */
}

.graph-container {
    width: 100%;
    overflow: visible !important;   /* prevents right-side clipping */
    padding-right: 20px;            /* extra space on right */
}

.button-center-box {
    width: 100%;
    display: flex;
    justify-content: center;   /* centers both buttons together */
    align-items: center;
    gap: 20px;                 /* spacing between PDF and Back buttons */
    margin: 20px 0;
}
  ...
  google.charts.load('current', {packages: ['corechart']});  
	...
	data.addColumn('number', 'Time');
      ...
      view.setColumns([0, 1, 1]);
        ...
        var options = {curveType: "function",width: document.getElementById('chart_wind').offsetWidth, height: 550,
        pointSize: 5,focusTarget: 'datum',
...
var x=$Each_Hours_GKwh_Arr.length 
      ...
      function drawChart() {
        ...
        var options = {curveType: "function", width: document.getElementById('chart_kwh').offsetWidth,height: 550,
       			 pointSize: 5,focusTarget: 'datum', colors: ['#007111'],
 ...
 <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
		  ...
		  ?>
				[<?=$PCAF_Key?>, parseInt(<?= $temp ?>)]<?=($C == ($Cur_Count-1)?'' : ',')?>
		  <?php
				$C++;
			}  
		  ?>
        ...
        var options = {curveType: "function",width: document.getElementById('chart_div').offsetWidth, height: 550,
        pointSize: 5,focusTarget: 'datum',
...
async function export_PDF() {
    ...
    const report = document.getElementById("body");
    ...
    pdf.setFontSize(16);
    pdf.text("Wind Turbine Performance: Power Curve Analysis", 10, 10);

    pdf.addImage(imgData, "JPEG", 10, 15, pageWidth, pageHeight);

    pdf.save("Power_Vs_WindSpeed_Report.pdf");
}
...
// Convert the data to binary form
  ...
  <center>
	  <div id="body" class="clear" style="width:98%; margin:auto;">
    <div class="box">
      <em class="tl"></em><em class="tr"></em><em class="bl"></em><em class="br"></em>
      <div class="content">
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <td  width="50%">
          <h2>Power Vs Wind Speed Curves</h2>
          
      </td>
	  <td width="30%"><h2><?=$Device_Name?></h2></td>
	  <div class="button-center-box">
	  
    <button class="download-btn" onclick="export_PDF()">Download PDF</button>
...
<div id="output"></div>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr><td align=right><input type=hidden name="Device_Name" id="Device_Name" value="<?= $Device_Name?>"><input type=hidden name="Owner_Name" id="Owner_Name" value="<?= $Firstname.' '.$Lastname?>">

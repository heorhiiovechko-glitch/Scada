...
//print_r($Mysql_Query1);
            ...
            if($Mysql_Query_Result[$Date_Range_Val_Final]->num_rows >= 1)
            {
                while($Fetch_Result[$Date_Range_Val_Final] = $Mysql_Query_Result[$Date_Range_Val_Final]->fetch_array()) {
						 ...
						 elseif($_REQUEST['p'] == 6){
       							 ?>
   							 <script>Open_PowerWindspeedCurveSendan('<?=$_REQUEST['c1']?>','<?=$From_D_Epoch?>','<?=$To_D_Epoch?>','<?=$Pocket_Length?>');</script> 
							 <?php  
						} 
						elseif($_REQUEST['p'] == 55){
								 ...
								 <script>Open_PowerWindspeedCurveSendanByDay('<?=$_REQUEST['c1']?>','<?=$From_D_Epoch?>','<?=$To_D_Epoch?>','<?=$Pocket_Length?>');</script> 

                    ...
                    <tr >
                        ...
                        <td align="left" style="padding-left:5px;">
                          <select name="p" id="p" style="width:180Px; padding-left:5px" onChange="For_Service_Report(this.value)">
								...
								<option value="19" <?=($_REQUEST['p'] == 19?'selected=selected' : '')?>>Power and Windspeed curves</option><?php
                         ...
                         <td align="left">
							...
							<?php
								if($_REQUEST['p'] == 13 || $_REQUEST['p'] == 27){
        ...
        <tr>
						...
						if(isset($_REQUEST['p'])){
							...
							if( $_REQUEST['p'] == 6){
								...
								foreach($Date_Range as $Date_Range_Val){
									...
									$Mysql_Query1[$Date_Range_Val_Final] = "select DISTINCT(SUBSTRING(Time_S,1,2)) AS Time_24 ,Time_S, Date_S,Record_Index from $Cook_Variable[7].device_data_f4 where IMEI = '".$IMEI."' and Date_S = '".$Date_Range_Val_Final1."' and (PAT_Gen1 != '') and PAT_Gen1 >'1' group by Time_24 order by Time_24 asc";
            ...
            if($Mysql_Query_Result1[$Date_Range_Val_Final]->num_rows >= 1)
            {
                while($Fetch_Result1[$Date_Range_Val_Final] = $Mysql_Query_Result1[$Date_Range_Val_Final]->fetch_array()) {		$Fetch_Result2 = $Fetch_Result1[$Date_Range_Val_Final];
									...
									#
									$Mysql_Query[$Date_Range_Val_Final] = "select Time_S, Date_S,PAT_Gen1,PAT_Gen2,Record_Index from $Cook_Variable[7].device_data_f4 where IMEI = '".$IMEI."' and Date_S = '".$Date_Range_Val_Final1."' and (PAT_Gen1 != '') and PAT_Gen1 >'1' order by Record_Index asc";// limit 31,18";

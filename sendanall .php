								...
								FROM device_register AS t1 
 								 LEFT JOIN
						            ...
						            SELECT Device_Index,State, COUNT(State)  totalCount
           							 FROM  device_register where Parent_ID='100253' GROUP   BY Region
       					 )  s ON s.State = t1.State where t1.Parent_ID='100253' group by IMEI   
...
$Tower_Img = '<img src="./images/6.gif" width="69px" height="98px" alt="Orange Tower">';
			...
			$PreviousState=$CurrentState;
							...
							if($CurrentState=="" || $CurrentState!=$Region["$IMEI"])
							{
								...
								?>
								<tr>
								<td valign="top" align="left"  style="font-size:15px; font-weight:bold"; ><?= substr($Region["$IMEI"],0,3) ?></td>
								</tr>
								<tr>
								 <?php
						...
						<tr>
							<td align="left"><a href=" " border="0" onclick="return false" style="cursor:pointer; color:#333333; text-decoration:none;" target="_blank"; title="<?=$DeviceName[$IMEI]?> &nbsp;<?=$Date_S?>,<?=$Time_S?>,&nbsp;<?=$Site_Location[$IMEI]?>,&nbsp;<?=$G1?>, &nbsp;<?=$G2?>"><?=$Div_Img?></a></td>
						</tr>

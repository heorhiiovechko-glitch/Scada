...
<?php
			...
			$Fetch_Info = "select a.HTSC_No,a.LOC_No,a.WEG_No,a.Group_Name,b.Firstname, a.Site_Location as Site_Location,a.SF_No as SF_No, a.Capacity as Capacity, a.Date_Of_Commission as Date_Of_Commission,a.Connect_Feeder as Connect_Feeder,a.Device_Name as Device_Name  from device_register a,user_master b where a.Account_ID = b.Account_ID and IMEI = '".$Query_IMEI."'";
			//echo $Fetch_Info;
...
<?
	...
	if (isset($_REQUEST["XLS"])){$XLS=1;}else{$XLS=0;}
        ...
        if ($XLS == 1){
               ...
               $reportName = "Daily_EB_Slot_Group_Report_" . date("d_M_Y") . ".xls";
	 ...
	 ?>   
        <script>
			function Date_Valid(){
				...
				if(Pselect == 12 || Pselect == 13 || Pselect == 16 || Pselect == 34){
...
<script>
/* FORCE HEADER COLOR AFTER REPORT LOAD */

window.addEventListener("load", function () {

    var headers = document.querySelectorAll(".tab-head-frame");

    headers.forEach(function (el) {

        // Remove inline background
        el.style.background = "";
        el.style.backgroundColor = "#0f4c81";
                        ...
                        <td align="left" style="padding-left:5px;">
                          <select name="p" id="p" style="width:180Px; padding-left:5px" onchange="For_Service_Report(this.value)">
                               <option value="1" <?=($_REQUEST['p'] == 1?'selected=selected' : '')?>>Power Vs Wind Speed</option>
								...
								?>
								<?php
									if($Cook_Variable[3]=="100316") {
								?>	
										<option value="60" <?=($_REQUEST['p'] == 60?'selected=selected' : '')?>>Performance Curve</option>
...
$IMEI = base64_decode($_REQUEST['c1']);
	...
	$Mysql_Query1[$Date_Range_Val_Final] = "select DISTINCT(SUBSTRING(Time_S,1,2)) AS Time_24 ,Time_S, Date_S,Record_Index from $Cook_Variable[7].device_data_f3 where IMEI = '".$IMEI."' and Date_S = '".$Date_Range_Val_Final1."' and (Production_Total != '') and Production_Total >'1' group by Time_24 order by Time_24 asc";
	...
	if (!$Mysql_Query_Result1[$Date_Range_Val_Final] = $db->query($Mysql_Query1[$Date_Range_Val_Final]))
            ...
            if($Mysql_Query_Result1[$Date_Range_Val_Final]->num_rows >= 1)
            {
                while($Fetch_Result1[$Date_Range_Val_Final] = $Mysql_Query_Result1[$Date_Range_Val_Final]->fetch_array()) {
									...
									#
									$Mysql_Query[$Date_Range_Val_Final] = "select Time_S, Date_S,Record_Index,Production_Total,Import_Kwh from $Cook_Variable[7].device_data_f3 where IMEI = '".$IMEI."' and Date_S = '".$Date_Range_Val_Final1."' and (Production_Total != '') and Production_Total >'1' order by Record_Index asc";// limit 31,18";
									//echo $Mysql_Query[$Date_Range_Val_Final];
//print_r($Mysql_Query);
			...
			$Fetch_Result3 = $Fetch_Result[$Date_Range_Val_Final];
											...
											//$PAT_Total[$Date_Range_Val_Final][$MI]/*["k".$Time_First_Seg]*/ = $Fetch_Result3['G1_Kwh'] + $Fetch_Result3['G2_Kwh'];
											$PAT_Total[$Date_Range_Val_Final][$MI]/*["k".$Time_First_Seg]*/ = $Fetch_Result3['Production_Total'];
									...
									//echo "Pat Total Output ===> ";print_r($Total_Count_DB[$Date_Range_Val_Final]);
									//echo "<br /><hr><br />";
								} // foreach end	
				 ...
				 elseif($_REQUEST['p'] ==60){
       							 ?>
   							  <script>Open_PowerWindspeedCurveClarion('<?=$_REQUEST['c1']?>','<?=$From_D_Epoch?>','<?=$To_D_Epoch?>','<?=$Pocket_Length?>');</script> 
							 <?php  
					...
					elseif( $_REQUEST['p'] == 37){
							...
							{
								$Mysql_Query = "select * from $Cook_Variable[7].device_data_f3 where IMEI = '".$IMEI."' and (Date_S >= '".$From_YMD."' and  Date_S <= '".$To_YMD."')  order by Record_Index desc";
								//echo "$Mysql_Query";
            ...
            if($Mysql_Query_Result->num_rows >= 1)
            {
                ...
                while($Fetch_Result = $Mysql_Query_Result->fetch_array()) {
										$All_Date_Arr[$MI] = $Fetch_Result['Date_F'];
										$All_Time_Arr[$MI] = $Fetch_Result['Time_F'];
										...
										if($MI == $Mysql_Record_Count){
											$L_G1_Kwh  = $Fetch_Result['PAT_Gen1'];
											$L_G2_Kwh  = $Fetch_Result['PAT_Gen2'];
											$L_G1_Hours  = $Fetch_Result['Gen1_Hours'];
											$L_G2_Hours  = $Fetch_Result['Gen2_Hours'];
    ...
    ?>
		...
		<?php
			...
			<?php
				...
				#
				if($_REQUEST['p'] == 6){
					include("Daily_EB_Report_Excel_Type3.php");
				}	
			?>
			<?php
						#
			...
			<?php
				...
				if($_REQUEST['p'] == 9){
					if ($Cook_Variable[6] == 100079 || $Cook_Variable[3] == 100081 || $Cook_Variable[3] == 100082 || $Cook_Variable[3] == 100084 ||  $Cook_Variable[3] == 100088 ) {
						include("Alarm_Log_Report_Type3_Lucky.php.php");
					} else {
...
<style>
/* =====================================================
   FORCE HEADER COLOR - BLOCK ALL GREEN OVERRIDES
   ===================================================== */

.tab-head-frame,
td.tab-head-frame,
tr td.tab-head-frame,
table td.tab-head-frame,
.innertab1 td.tab-head-frame,
.innertab1 tr td.tab-head-frame {

    background: #0f4c81 !important;   /* Blue */
...
/* Prevent JS / Hover / Active Changes */
.tab-head-frame:hover,
.tab-head-frame:focus,
.tab-head-frame:active {
    background: #0f4c81 !important;
    background-color: #0f4c81 !important;
    color: #ffffff !important;
}

/* Also lock column headers */
.tab-head-td,

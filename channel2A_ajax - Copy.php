            ...
            <td valign="top">
                <table width="100%" border='0' align="left" cellpadding="1" cellspacing="1" class="innertab1">	
                    ...
                    <tr >
                        ...
                        <td align="left" style="padding-left:5px;">
                            <select name="p" id="p" style="width:180Px; padding-left:5px" onChange="For_Service_Report_Type6(this.value)">
								...
								<option value="8" <?=($_REQUEST['p'] == 8?'selected=selected' : '')?>>Alarm Log </option>
										...
										<option value="9" <?=($_REQUEST['p'] == 9?'selected=selected' : '')?>>DGR Grouping Report</option>
                        ...
                        <td align="center"><input type="hidden" name="l" value="<?=$_REQUEST['l']?>" />							<input type="hidden" name="FType" value="<?=$_REQUEST['FType']?>" /><input type="hidden" name="c1" value="<?=$_REQUEST['c1']?>" /><input type="submit" name="dateSearch" id="dateSearch" value="GO">
    ...
    ?>
		...
		<?php
		 ...
		 if($_REQUEST['p'] == 8){
				...
				$Fetch_Info = "select a.HTSC_No,a.LOC_No,a.WEG_No,a.Group_Name, a.Site_Location as Site_Location,a.SF_No as SF_No, a.Capacity as Capacity, a.Date_Of_Commission as Date_Of_Commission,a.Connect_Feeder as Connect_Feeder,a.Device_Name as Device_Name  from va_master.device_register a Where IMEI = '".$Query_IMEI."'";
					...
					if($Fetch_Info_Result->num_rows >= 1)
					{
						...
						$ADevicename = $All_Devicename[$x];
						  
						//echo  $All_Devicename[$x];	
						$x++; }
						  
					}
					
					$filePath = $_SERVER['DOCUMENT_ROOT'] . "\\2023\\Export\\va_karma\\". $ADevicename. "_alarmlog.xls";
					...
					if(!empty($filename)){
						...
						{
							echo '<a href="' . "http://versatilescada.com/2023/Export/va_karma/". $ADevicename. "_alarmlog.xls" . '">Download Data </a>';
					...
					//echo '<a href="' . "http://versatilescada.com/2023/Export/va_karma/". $ADevicename. "_alarmlog.xlsx" . '">Download Data </a>';
			...
			if($_REQUEST['p'] == 9){
				...
				$Fetch_Info = "select a.HTSC_No,a.LOC_No,a.WEG_No,a.Group_Name, a.Site_Location as Site_Location,a.SF_No as SF_No, a.Capacity as Capacity, a.Date_Of_Commission as Date_Of_Commission,a.Connect_Feeder as Connect_Feeder,a.Device_Name as Device_Name  from va_master.device_register a Where IMEI = '".$Query_IMEI."'";
					...
					$filePath = $_SERVER['DOCUMENT_ROOT'] . "\\2023\\Export\\va_karma\\". $ADevicename. "_DGR.xlsx";
					$filename = basename($filePath);
					// Specify file path.
					//$path = ''; // '/uplods/'
					//$download_file =  $path.$filename;
					...
					if(!empty($filename)){
						...
						if(file_exists($filePath))
						{
							echo '<a href="' . "http://versatilescada.com/2023/Export/va_karma/". $ADevicename. "_DGR.xlsx" . '">Download Data </a>';

...
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(PAT_GEN2,0)) as PAT_GEN2_Max ,MIN(GREATEST(PAT_GEN2,0)) as PAT_GEN2_Min ,ROUND(AVG(GREATEST(POWER,0)),2) as Power, ROUND(AVG(WINDSPEED),2) as WindSpeed, Date_S  from $Cook_Variable[7].device_data where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and PAT_GEN2 between 1 and 20000000  group by day(Date_S)";
elseif($Format_Type==2)
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(PAT_GEN1,0)) as G1_Kwh_Max,MIN(GREATEST(PAT_GEN1,0)) as G1_Kwh_Min,MAX(GREATEST(PAT_GEN2,0)) as G2_Kwh_Max,MIN(GREATEST(PAT_GEN2,0)) as G2_Kwh_Min,round(AVG(GREATEST(POWER,0)),2)   as Power ,ROUND(AVG(WINDSPEED),2) as WindSpeed ,Date_S from $Cook_Variable[7].device_data_f2 where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and PAT_Gen1 between 1 and 10000000 and PAT_Gen2 between 1 and 10000000 group by day(Date_S)";
elseif($Format_Type==4)
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(PAT_GEN1,0)) as G1_Kwh_Max,MIN(GREATEST(PAT_GEN1,0)) as G1_Kwh_Min,MAX(GREATEST(PAT_GEN2,0)) as G2_Kwh_Max,MIN(GREATEST(PAT_GEN2,0)) as G2_Kwh_Min,round(AVG(GREATEST(POWER,0)),2)   as Power ,ROUND(AVG(WINDSPEED),2) as WindSpeed ,Date_S from $Cook_Variable[7].device_data_f4 where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and PAT_Gen1 between 1 and 10000000 and PAT_Gen2 between 1 and 10000000 group by day(Date_S)";
elseif($Format_Type==6)
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(PAT_GEN2,0)) as PAT_GEN2_Max ,MIN(GREATEST(PAT_GEN2,0)) as PAT_GEN2_Min ,ROUND(AVG(GREATEST(POWER,0)),2) as Power, ROUND(AVG(WINDSPEED),2) as WindSpeed, Date_S from $Cook_Variable[7].device_data_f6 where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and PAT_GEN2 between 1 and 20000000  group by day(Date_S)";
elseif($Format_Type==10)
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(Production_Total,0)) as GEN_Max ,MIN(GREATEST(Production_Total,0)) as GEN_Min ,ROUND(AVG(GREATEST(POWER,0)),2) as Power, ROUND(AVG(WINDSPEED),2) as WindSpeed, Date_S from $Cook_Variable[7].device_data_f10 where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and Production_Total between 1 and 20000000  group by day(Date_S)";
elseif($Format_Type==3)
$Mysql_Query="select Day(Date_S) as Day,MAX(GREATEST(Production_Total,0)) as GEN_Max ,MIN(GREATEST(Production_Total,0)) as GEN_Min ,ROUND(AVG(GREATEST(Power,0)),2) as Power, ROUND(AVG(Windspeed),2) as WindSpeed, Date_S as Date_S  from $Cook_Variable[7].device_data_f3 where IMEI = '".$IMEI."' and month(Date_S)=".$MonthYearArr[0]." and Year(Date_S)=".$MonthYearArr[1]." and Production_Total between 1 and 20000000  group by day(Date_S)";
		//echo $Mysql_Query;
		if (!$Mysql_Query_Result = $db->query($Mysql_Query))
            {
                die($db->error);
		...
		if($Format_Type==1 || $Format_Type==6){
			...
			foreach($Each_Days_PAT_GEN2_Max_Arr as $PAT_GEN2_Max_Key => $PAT_GEN2_Max_Value){
				...
				if($arrayiteratorcount==0)
					...
					$Each_Days_Generation_Arr[$PAT_GEN2_Max_Key]=$PAT_GEN2_Max_Value-$PAT_GEN2_Min_Value;
			...
			foreach($Each_Days_GEN_Max_Arr as $GEN_Max_Key => $GEN_Max_Value){
				...
				if($arrayiteratorcount==0)
					...
					$Each_Days_Generation_Arr[$GEN_Max_Key]=$GEN_Max_Value-$GEN_Min_Value;
			...
			foreach($Each_Days_G1_Kwh_Max_Arr as $G1_Kwh_Max_Key => $G1_Kwh_Max_Value){
				...
				if($arrayiteratorcount==0)
					...
					$Each_Days_G1_Kwh_Arr[$G1_Kwh_Max_Key]=$G1_Kwh_Max_Value-$G1_Kwh_Min_Value;
			...
			foreach($Each_Days_G2_Kwh_Max_Arr as $G2_Kwh_Max_Key => $G2_Kwh_Max_Value){
				...
				if($arrayiteratorcount==0)
					...
					$Each_Days_G2_Kwh_Arr[$G2_Kwh_Max_Key]=$G2_Kwh_Max_Value-$G2_Kwh_Min_Value;
					$G2_Kwh_Min_Value=$G2_Kwh_Max_Value;
				$arrayiteratorcount++;
			}//endforeach		
			$Each_Days_Generation_Arr = array();
			for($i=0;$i<count($Each_Days_G1_Kwh_Arr);$i++) {
...
//power values stored in $Each_Hours_PowerData_Arr[$HA_Val][].power values -ve values marked as zero . windspeed values in $Each_Hours_WindSpeedData_Arr[$HA_Val][]
		...
		/*echo "<hr>";
		print_r($Each_Days_PowerData_Arr);
		//echo "<hr>";exit;*/
...
//print_r($Each_Hours_WindSpeedData_Arr);
		...
		$PCWP_Chart_Arr1 = array_combine($Each_Days_WindSpeedData_Arr_Avg,$Each_Days_PowerData_Arr_Avg);
...
<script>
  google.charts.load('current', {packages: ['corechart']});  
	...
	data.addColumn('number', 'Day');
   	data.addColumn('number', 'WindSpeed');
         ...
         data.addRows([
		  ...
		  <?php
			...
			$Cur_Count = count($Each_Days_WindSpeedData_Arr_Avg);
			foreach($Each_Days_WindSpeedData_Arr_Avg as $EHWA_Key => $EHWA_Val){
				
				//$temp=(int)$temp; 
		  ?>
				[ parseInt(<?= $EHWA_Key ?>) , parseInt(<?= $EHWA_Val ?>) ] <?=($C == ($Cur_Count-1)?'' : ',')?>
		  <?php
				$C++;
			}
		  ?>
      ...
      view.setColumns([0, 1, 1]);
        ...
        pointSize: 4,focusTarget: 'datum',
          title: ' Wind Speed Graph Detail for the month of <?=date("F", mktime(0, 0, 0, $Month, 10))?> ',
	...
	series: {0: { lineWidth: 2},1: {color: 'red',lineWidth: 0,pointSize: 5}},
          ...
          hAxis: {title: 'Days', titleTextStyle: {color: 'red'}, maxValue:31, minValue:1  ,titleTextStyle: {color: 'red',fontSize:18,  fontName:'Times New Roman'}, gridlines:{count:31},viewWindowMode:"explicit", viewWindow: {min:1,max:31}},
...
google.load("visualization", "1", {packages:["corechart"]});
google.setOnLoadCallback(drawChart); 
var x=$Each_Days_GKwh_Arr.length 
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
		  ...
		  <?php
			...
			$Cur_Count1 = count($Each_Days_Generation_Arr);
			foreach($Each_Days_Generation_Arr as $EHGA_Key1 => $EHGA_Val1){
				
				//$temp=(int)$temp; 
		  ?>
				[ parseInt(<?= $EHGA_Key1 ?>) ,  parseInt(<?= $EHGA_Val1 ?>) ] <?=($C1 == ($Cur_Count1-1)?'' : ',')?>
		  <?php
				$C1++;
			}
		  ?>
        ...
        var options = {curveType: "function",width: 900, height: 400,
          		 ...
          		 title: 'Generation Graph for the month of <?=date("F", mktime(0, 0, 0, $Month, 10)) ?>-------------Export - <?= $Each_Days_Generation_Arr_Sum?>kwh',
         		 titleTextStyle:  {color: 'black', fontName:'Times New Roman', fontSize: 18},
          	 	  legend: 'none',
			 backgroundColor:{stroke:'#999',strokeWidth:3},
          		 hAxis: {title: 'Days', titleTextStyle: {color: 'red'}, maxValue:31, minValue:1  ,titleTextStyle: {color: 'red',fontSize:18,  fontName:'Times New Roman'}, gridlines:{count:31}, viewWindowMode:"explicit", viewWindow: {min:1,max:31}},

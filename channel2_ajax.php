...
if ($XLS == 1) {
	...
	switch ($_REQUEST['p']) {
		...
		case 43:
			...
			break;
		case 50:
			$Title_Head = "Raw_Data_Quarter_Hourly_Production";
			break;
	}
...
/* Prevent Green Override */
.tab-head-frame[style],
.tab-head-td[style] {
    background: linear-gradient(135deg, #0f4da2, #1976d2) !important;
}
...
<body>
	<table width="100%" border='0' align="left" cellpadding="1" cellspacing="1" class="innertab1">
							...
							<form name="wind" onsubmit="return Date_Valid()">
								<td align="left" style="padding-left:5px;">
									<select name="p" id="p" style="width:180Px; padding-left:5px" onChange="For_Service_Report_Type6(this.value)">
										...
										<option value="50" <?= ($_REQUEST['p'] == 50 ? 'selected=selected' : '') ?>>15 min Production Report</option>
										<!--<option value="52" <?= ($_REQUEST['p'] == 52 ? 'selected=selected' : '') ?>>15 min Production group Report</option>-->
				...
				if (isset($_REQUEST['dateSearch'])) {
					...
					if (isset($_REQUEST['p'])) {
						...
						elseif ($_REQUEST['p'] == 20) {
							...
							?>
							<script>
								New_Win_Open2('<?= $_REQUEST['c1'] ?>', 'EB_Meter_Reading.php', '<?= $_REQUEST['inputMonthYear'] ?>', '<?= $FType ?>')
							</script>
						...
						?>
							<script>
								Open_PowerWindspeedCurve('<?= $_REQUEST['c1'] ?>', '<?= $From_D_Epoch ?>', '<?= $To_D_Epoch ?>', '<?= $Pocket_Length ?>')
							</script>
						<?php
						...
						?>
							<script>
								Open_PowerWindspeedCurveMonth('<?= $_REQUEST['c1'] ?>', '<?= $_REQUEST['inputMonthYear'] ?>', '<?= $Pocket_Length ?>')
							</script>
						<?php
						} elseif ($_REQUEST['p'] == 47) {
						?>
							<script>
								Open_PowerWindspeedRefCurve('<?= $_REQUEST['c1'] ?>', '<?= $From_D_Epoch ?>', '<?= $To_D_Epoch ?>', '<?= $Pocket_Length ?>')
							</script>
						<?php
						...
						?>
							<script>
								New_Win_Open3('<?= $_REQUEST['c1'] ?>', 'Invoice_Upload.php', '<?= $_REQUEST['inputMonthYear'] ?>', '<?= $Pocket_Length ?>')
							</script>
		...
		<?php
					#
					#	Quarter Hourly Production Report
					#
					if ($_REQUEST['p'] == 50) {
						include("Raw_Data_Quarter_Hourly_Production.php");
					}
		?>
		<?php
					#
		...
		<?php
					...
					if ($_REQUEST['p'] == 11) {
						...
						echo "<script>console.log('Debug Objects: ".$_REQUEST['p']." ' );</script>";
		...
		<?php
					#
					#	Daily Generation KWH Report-Grouping
					#
					if ($_REQUEST['p'] == 51) {

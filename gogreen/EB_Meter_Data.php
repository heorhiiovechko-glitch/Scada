<?php
	...
	$Form_Fields[] = array('1','1','Multiplication Factor','Multiplication_Factor','','','E|N|0','','txtbox','','','');
	
	
		//Submit Button
	$Form_Fields[] = array('1','5','',$Submit_Txt,'Add IMEI','','','','submit_but','');
	...
	if(isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])){
		...
		if($Record_Count == 0){
			if($Query_To_Mysql_Join == '')
				...
				if($Query_To_Mysql_Result){
					$Encoded_IMEI=base64_encode($_REQUEST['IMEI']);
					$Msg = urlencode("New Record $Encoded_IMEI  has been added successfully");

<?php
	...
	$Order_By = "Device_Index asc"; // Column name to be order and which order (asc or desc)
	...
	//$Form_Fields[] = array('1','1','From Date','From','','','E|N|0','','Filter_box','Cal_T','','F','Hid');
	//$Form_Fields[] = array('1','1','To Date','To','','','E|N|0','','Filter_box','Cal_T','','T','Hid');
	...
	//$Form_Fields[] = array('1','3','Vehicle Status','Manual_Status',$Vehicle_Status_Array,'','1','','Filter_Sel1','onchange="Filter_Func(this.id,this.value,'.$Title_Head1.')"','-----Vehicle Status-----','');

		
	// Search Area Customization	
	$Search_Fields = array('IMEI');
	...
	if( (isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])) || (isset($_REQUEST['SearchD'])) ){
		...
		if(isset($_REQUEST['search'])){
			...
			foreach($SearchD1 as $SearchD2){
				...
				if($Sub_Omit != "0O"){
					...
					if(($SearchD3[1] != '0') && ($SearchD3[0] != 'search')){
						...
						if(!empty($SearchD3[1])){
								if($SearchD3[0] != 'Page' && $SearchD3[0] != 'Sortby' && $SearchD3[0] != 'Sortto'){
			...
			if(trim($Where_Query) == 'where')
				$Where_Query = '';
			if($XLS == 1 && empty($_REQUEST['search'])){
				$Where_Query = str_replace("where SearchD = 'Yes'","",$Where_Query);	
				$SearchD1 = explode('&',$_SERVER['QUERY_STRING']);
				$SearchD1 = explode('=',$SearchD1[1]);
				$Where_Query= "where Manual_Status = '".urldecode($_REQUEST['Manual_Status'])."'";
			
			}
			$Query_To_Mysql_Search = "SELECT * FROM $Table_Name $Where_Query order by $Order_By";		
			//echo $Query_To_Mysql_Search = "SELECT * FROM $Table_Name $Where_Query and Account_ID = '".$_REQUEST['P_ID']."' order by $Order_By";  // add by saranya
...
?>			
				...
				<?php
					...
					elseif(isset($Query_To_Mysql_Join))
						$Select_Mysql = $Query_To_Mysql_Join;
					else	
						$Select_Mysql = "SELECT IMEI FROM $Table_Name order by $Order_By";
                ...
                ?>                
				<tr>
					<td align="right" colspan="<?=$Colspan_Count?>"><div class="total_admin">Total Installed Vehicles : <?=$Total_Records?><br /></div></td>	
				</tr>
				<?
                } // XLS == 0 end
                ?>
						...
						foreach($List_Fields as $Fields){
							if($XLS == 0){
								...
								if($Fields[1] == 'Zone_No'){
									...
									echo '<td class="admin-menu-heading"><div align="left"><a href="javascripr:void()" title="Sort"><strong>Device ID</strong>&nbsp;</a></div></td>';
								}
								else{
									echo '<td class="admin-menu-heading"><div align="left"><a href="'.$List_Head_Url.'" title="Sort"><strong>'.$Fields[0].'</strong>&nbsp;'.($Fields[1] == $_REQUEST['Sortby']?$arrows : '').'</a></div></td>';
								}	
				...
				<?php
					...
					while($Fetch_Result = mysqli_fetch_array($Result)){
						...
						($Record_Column%2 == 1 ? $Row_Cls = 'Grey_Bg' : $Row_Cls = 'White_Bg');
						// Serial No increment for pagination
						$SNo1 = ($SNo + (($Records_Per_Page * $_REQUEST['Page']) - $Records_Per_Page));
						echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
							echo "<td class='Row_Td' style='padding-right:5px;' width='20px'>$SNo1</td>";	
							foreach($List_Fields as $Fields){
									...
									elseif($Fields[0] == 'Log File'){
										echo "<td class='Row_Td'>";
											$Day_Before_Yesterday = date("d_m_Y",strtotime("-2 days"));	
											$Yesterday= date("d_m_Y",strtotime("-1 days"));	
											$Current_Day = date("d_m_Y");	
											
											$File_pa = $File_SerialLog_Path;
											$File_DBY = "SERIAL_DATA_".$Day_Before_Yesterday.".txt";
											$Check_DBY = $File_pa."/".$File_DBY;
											$File_Y = "SERIAL_DATA_".$Yesterday.".txt";
											$Check_Y = $File_pa."/".$File_Y;
											$File_C = "SERIAL_DATA_".$Current_Day.".txt";
											$Check_C = $File_pa."/".$File_C;
												
											echo $View_Link = "<table><tr>";
											if(file_exists($Check_C)){
												echo "<td><a href='".$Admin_Path."/".$File_C."' title=\"View Today Log Information\" target=\"_New\" $class>T</a>&nbsp;&nbsp;|&nbsp;&nbsp;</td>";
											}	
											if(file_exists($Check_Y)){
												echo "<td><a href='".$Admin_Path."/".$File_Y."' title=\"View Yesterday Log Information\" target=\"_New\" $class>Y</a>&nbsp;&nbsp;|&nbsp;&nbsp;</td>";
											}
											if(file_exists($Check_DBY)){
												echo "<td><a href='".$Admin_Path."/".$File_DBY."' title=\"View Day Before Yesterday Log Information\" target=\"_New\" $class>DY</a></td>";
											}
											echo "</tr></table>";
										echo "</td>";
									}

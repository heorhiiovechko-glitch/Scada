<?php
	...
	$List_Fields[] = array('Log File','Account_ID','','','');
...
?>			
				...
				<?php
					...
					if($Query_To_Mysql_Search)
						...
						$Select_Mysql = "SELECT $Edit_Column $Mysql_Select_Column FROM $Table_Name";
				...
				<?php
					if($XLS == 0){
						$Logs_Folder_Files_Arr = Read_Direcotry_Bmw($Serial_Logs_File_Path_Raw);
						ksort($Logs_Folder_Files_Arr);
						$Logs_Folder_Files_Arr = array_reverse($Logs_Folder_Files_Arr); 
						$Files_Count = count($Logs_Folder_Files_Arr);
					// Currently display Records
					$Result = $Pager->paginate();
					foreach($Logs_Folder_Files_Arr as $Fetch_Result){
					...
					//for($F = 0;$F < $Files_Count; $F++){
						...
						echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
							...
							foreach($List_Fields as $Fields){
									...
									if($Fields[0] == 'Date'){
										$Date_Of_the_File1 = str_replace('SERIAL_DATA_','',$Fetch_Result);
										$Date_Of_the_File2 = str_replace('.txt','',$Date_Of_the_File1);
										$Date_Of_the_File = str_replace('_','-',$Date_Of_the_File2);
									echo "<td class='Row_Td'>";
										echo date("d-M-Y",strtotime($Date_Of_the_File));
										echo "</td>";
									}
									...
									elseif($Fields[0] == 'Log File'){
										echo "<td class='Row_Td'>";
											...
											$Serial_Logs_Path = $Serial_Log_Location."/".$Fetch_Result;

											echo $View_Link = "<table><tr>";
												echo "<td><a href='".$Serial_Logs_File_Path_Link."".$Serial_Logs_Path."' title=\"View Today Log Information\" target=\"_New\" $class>".$Fetch_Result."</a></td>";
											echo "</tr></table>";
										echo "</td>";
									}

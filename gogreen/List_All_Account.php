<?php
	...
	#   $Date_Format = 5 - 10-10-2011 (d-m-Y)
	#   $Date_Format = 6 - 10-Feb-2011 (d-M-Y)
	#
	#	// For Select Event
	...
	//$Form_Fields[] = array('1','3','Account','Account_ID',$Firstname_Admin_Array,'','1','','Filter_Sel2','onchange="Filter_Func(this.id,this.value,'.$Title_Head1.')"','----- Account Details -----','');

		
	// Search Area Customization	
	$Search_Fields = array('Firstname');
	...
	if( (isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])) || (isset($_REQUEST['SearchD'])) ){
		...
		if(isset($_REQUEST['SearchD'])){
			...
			$Where_Query= "where ".$SearchD1[0]." = ".$SearchD1[1]."";
		}	
...
?>			
                ...
                ?>                
				<tr>
					<td align="right" colspan="<?=$Colspan_Count+1?>"><div class="total_admin">Total Records : <?=$Total_Records?><br /></div></td>	
				</tr>
				<?
                } // XLS == 0 end
                ?>
				...
				<tr bgcolor="#F0F0F0" height="25px">
					...
					<?
						foreach($List_Fields as $Fields){
							if($XLS == 0){
								echo '<td class="admin-menu-heading"><div align="left"><a href="'.$List_Page.'.php?Page='.$_REQUEST['Page'].'&Sortby='.$Fields[1].'&Sortto='.$Sortto.'" title="Sort"><strong>'.$Fields[0].'</strong>&nbsp;'.($Fields[1] == $_REQUEST['Sortby']?$arrows : '').'</a></div></td>';
							}
							if($XLS == 1){
					...
					?>
						<td class="admin-menu-heading"><div align="left"><a href="" title="Sort"><strong>#</strong></a></div></td>
			  	</tr>
				<tr>
							...
							$Excel_Output.= "<tr height='30'>";
								...
								foreach($List_Fields as $Fields){
										...
										echo "<td style='font-size:12px;text-align:left'>";
											echo ($Fields[2]?$Fetch_Result[$Fields[1]] : (($Fields[3]?Date_Format_Func($Fetch_Result[$Fields[1]],$Fields[3]) : $Fetch_Result[$Fields[1]])) );
										echo "</td>";	
								}								
								echo "</tr>";
				...
				<?php
					...
					while($Fetch_Result = mysql_fetch_array($Result)){
						...
						echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
							...
							foreach($List_Fields as $Fields){
	$List_Fields1[] = array($Column_Name,$Column_Heading,$Content_Limit,$Date_Format);

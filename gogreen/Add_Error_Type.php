<?php
	...
	#	
	$Mysql_Query = "select * from DEVICE_REGISTER group by IMEI";
	...
	if($Mysql_Record_Count>=1){
		while($Fetch_Result = mysql_fetch_array($Mysql_Query_Result)){
			$All_IMEIS_Array[$Fetch_Result['IMEI']] = $Fetch_Result['IMEI'];
		}
	}
	...
	//Text boxes
	//Select
	$Form_Fields[] = array('2','2','IMEI','IMEI',$All_IMEIS_Array,'*','1','selectcls','','-----Select IMEI-----','');
	$Form_Fields[] = array('1','1','Error Message','Error','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','9','Sample Raw Data','Raw_Data','','*','E|N|0','style="height:70px;"','txtarea','','');
	$Form_Fields[] = array('1','7','Date Stamp','Date_Stamp',date("Y-m-d H:i:s"),'*','E|N|0','','txtbox','','');
	//Submit Button
	$Form_Fields[] = array('1','5','',$Submit_Txt,'Add Error','','','','submit_but','');


	/*$Form_Fields[] = array('1','1','To_Date','To_Date','','*','E|N|0','','txtbox','Cal_T','');
	$Form_Fields[] = array('1','2','Password','Pword','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','3','Radio','Rad','Male|FeMale','*','E|N|0','','radio_txt','','');
	$Form_Fields[] = array('1','4','Checkbox','Chec','yes','*','E|N|0','','check_txt','','');
	$Form_Fields[] = array('1','7','Hidden','Hid',date("Y-m-d H:i:s"),'*','E|N|0','','txtbox','','');
	
	$Form_Fields[] = array('1','8','File','Fil','','*','E|N|0','','txtbox','','');
	$Photo_Fields['Fil'] = array("Fil","biFile","",'','','Yes','F');

	$Form_Fields[] = array('1','8','File','Fil2','','*','E|N|0','','txtbox','','');
	$Photo_Fields['Fil2'] = array("Fil2","biFile","",'','','Yes','F');

	$Form_Fields[] = array('1','9','Textarea','Texta','','*','E|N|0','','txtarea','','');

	//$Form_Fields[] = array('2','2','DropDown 1','Gender',$Gender_Array,'*','1','selectcls','','-----Gender-----','');
	...
	if(isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])){
		...
		if($Record_Count == 0){
			if($Query_To_Mysql_Join == '')
				echo $Query_To_Mysql = "Insert into $Table_Name (".substr($Query_Left,0,-1).") values (".substr($Query_Right,0,-1).")";
			else

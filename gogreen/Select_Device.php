...
<?php
	...
	#	// For Select Event
	#	$Form_Fields[]= array($Element_Type,$Field_Type,$Field_Heading,$Field_Name,$Field_Value,$Manditary,$JValid,$Class_Name,$Extra,$Combo_Title,$Match,$label);
	#	// For Select Example
	...
	if(empty($_COOKIE[$Cook_Name])){
		...
		# Getting IMEI
		$Mysql_Query = "select Device_Name,IMEI from device_register order by db_name";
...
//echo $Device;
		...
		/*if($Record_Count == 0){
			if($Query_To_Mysql_Join == '')
				$Query_To_Mysql = "update device_register set Closing_Time='$Closing_Time' where IMEI='".$Device."'";
			else
				...
				if($Query_To_Mysql_Result){
					...
					$Msg = urlencode("Closing Time has been added successfully");
...
$Query_Date="Date_S='$Date'";														
			...
			$Resultset=mysqli_query($Device_Query);
			$Row_Num=mysqli_num_rows($Resultset);
			if($Row_Num>=1){
if($Format_Type == 1) {
	header("Location:Insert_Raw_Data_Type1.php?FT=".$Format_Type."&db=".$db_name."&imei=".$IMEI."&Date=".$Date."");
}
elseif($Format_Type == 2) {
	header("Location:Insert_Raw_Data_Type2.php?FT=".$Format_Type."&db=".$db_name."&imei=".$IMEI."&Date=".$Date."");
}
elseif($Format_Type == 3) {
	header("Location:Insert_Raw_Data_Type3.php?FT=".$Format_Type."&db=".$db_name."&imei=".$IMEI."&Date=".$Date."");
}
elseif($Format_Type == 6) {
	header("Location:Insert_Raw_Data_Type6.php?FT=".$Format_Type."&db=".$db_name."&imei=".$IMEI."&Date=".$Date."");
}
if($Format_Type == 10) {
	header("Location:Insert_Raw_Data_Type10.php?FT=".$Format_Type."&db=".$db_name."&imei=".$IMEI."&Date=".$Date."");
}
...
<?php 
		echo "<form action=\"\" method=\"post\" name=\"".$Title_Head."\" onsubmit=\"return FormValid('$Jvalid_Arr_Join','$Jvalid_Type_Arr_Join')\" enctype=\"multipart/form-data\" >";
				echo "<div id=\"admin_left\" style=\"width:990px; border:solid 0px red;\">".$_REQUEST[msg]."<p>&nbsp;</p>";
				echo '
				...
				// For select Box
					...
					<div id="admin_left_inner">
						...
						<?=Func_Select_Element($Forms[0],$Forms[1],$Forms[3],$Forms[4],$Forms[7],$Forms[8],$Forms[9],$Forms[10],$Forms[11]);?>
						<?=J_Mes($Forms[3]);?>
					</div>

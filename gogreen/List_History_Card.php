<?php
	...
	$Order_By = "H_Id desc"; // Column name to be order and which order (asc or desc)
	$Records_Per_Page = 20;
	$Pagination_No_Count = 2;
	$Edit_ID_Column = "H_Id"; // Unique value should be give for whatever query you do in that table
	...
	$Search_Fields = array('IMEI','Date_Stamp','Name of customer');

	//List Fields
	$List_Fields[] = array('IMEI','IMEI','','','');
	$List_Fields[] = array('Name of Customer','H1','','','');	
	$List_Fields[] = array('Location','H3','','','');
	$List_Fields[] = array('Created Date','Date_Stamp','','','');
	...
	if( (isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])) || (isset($_REQUEST['SearchD'])) ){
		...
		if(isset($_REQUEST['search'])){
			...
			if($_REQUEST['search'] != '')
				$Where_q = " and";
			$Where_Query= "where Parent_ID != '' ".$Where_q." ".$Where_Query_Combind."";
				...
				<?php
					...
					while($Fetch_Result = mysql_fetch_array($Result)){
						...
						echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
							...
							echo "<a href='Add_Device.php?P_ID=".$Fetch_Result['Account_ID']."' title='Add Device'><img src='./images/icon_question.gif' height='20px' width='20px' title='Add Device'></a>&nbsp;&nbsp;&nbsp;&nbsp;";

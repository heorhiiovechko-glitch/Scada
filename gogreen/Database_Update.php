...
//$Mysql_Query = "SELECT Distinct IMEI,Account_ID,Format_Type,Device_Name,hour(Closing_Time) as Closing_Time,db_name from va_master.device_register order by db_name,Format_Type";
$Mysql_Query = "SELECT Distinct IMEI,Account_ID,Format_Type,Device_Name,hour(Closing_Time) as Closing_Time,db_name from va_master.device_register where db_name in ('va_jayakrishna','va_ssm','va_dmr','va_vinayaka','va_reinfra','va_dhanalakshmi','va_osgreen','va_rsm','va_balajispinning') order by db_name,Format_Type";
//echo $Mysql_Query ;
...
foreach($POC_IMEI as $IMEI_Val){
							...
							//$Date_Stamp="2019-02-14";
							if($Closing_Time[$IMEI_Val]=='00'){
							$Yester_Stamp=$Date_Stamp;
							
							}
							else{
							$Yester_Stamp=date("Y-m-d",strtotime("-2 days"));
							//$Yester_Stamp="2019-02-13";
							
							}
print_r($Closing_Time[$IMEI_Val]);
...
//print_r($row['Run']);	
	...
	if ($Data_Query_Result){
  $Insert_Query="Insert into $DB_Name[$IMEI_Val].daily_generation_data (IMEI,Windspeed,Power,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Run_Hours,Gen1_Hours,Line_Ok,Gen0,Date_S) values('$IMEI_Val','$WindSpeed','$Power','$Gen1_Min','$Gen1_Max','$Gen2_Min','$Gen2_Max','$Run','0','0','$Gen0','$Date_S')";
$Insert_Query_Result = $db->query($Insert_Query);
}			
...
if ($Data_Query_Result){
  $Insert_Query="insert into $DB_Name[$IMEI_Val].daily_generation_data (IMEI,Windspeed,Power,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Run_Hours,Gen1_Hours,Line_Ok,Gen0,Date_S) values('$IMEI_Val','$WindSpeed','$Power','$Gen1_Min','$Gen1_Max','$Gen2_Min','$Gen2_Max','$Run','0','0','0','$Date_S')";
echo $Insert_Query;
$Insert_Query_Result = $db->query($Insert_Query);
}
...
//print_r($row['Run']);	
    	...
    	if ($Data_Query_Result){
  $Insert_Query="Insert into $DB_Name[$IMEI_Val].daily_generation_data (IMEI,Windspeed,Power,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Run_Hours,Gen1_Hours,Line_Ok,Gen0,Date_S) values('$IMEI_Val','$WindSpeed','$Power','$Gen1_Min','$Gen1_Max','0','0','$Run','$Gen1','0','$Gen0','$Date_S')";
echo $Insert_Query;
$Insert_Query_Result = $db->query($Insert_Query);
}

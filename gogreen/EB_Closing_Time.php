...
<?php
	...
	if(empty($_COOKIE[$Cook_Name])){
		...
		# Getting IMEI
		$Mysql_Query = "select * from device_register order by db_name";
		...
		if($Mysql_Record_Count>=1){
			while($Fetch_Result = mysqli_fetch_array($Mysql_Query_Result)){
				$ALLDB_Array[$Fetch_Result['IMEI']] = $Fetch_Result['db_name'];
				$ALLDevice_Array[$Fetch_Result['IMEI']] = $Fetch_Result['Device_Name'];
				//$DbName[$Fetch_Result['db_name']] = $Fetch_Result['db_name'];
			}
		}
	...
	//Text boxes
	$Form_Fields[] = array('2','2','Device Name','Device_Name',$ALLDB_Array,'*','1','selectcls','','----- Select Device-----','',$ALLDevice_Array);
	
	$Form_Fields[] = array('1','7','Parent_ID','Parent_ID','','','E|N|0','','txtbox','','','');
	
	$Form_Fields[] = array('2','2','Closing Hour','Closing_Hour',$Hour24_Array,'*','1','selectcls','','----- Select Hour-----','','');
	
	
		//Submit Button
	$Form_Fields[] = array('1','5','',$Submit_Txt,'Set Closing Time','','','','submit_but','');
	...
	if(isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])){
		...
		$Closing_Time=$_REQUEST['Closing_Hour'];
...
$(document).ready(function(){
  $("#Account_ID").change(function(){
    $('#Parent_ID').val($("##Account_ID option:selected").attr('label'));
  });
});
</script>
<?php 

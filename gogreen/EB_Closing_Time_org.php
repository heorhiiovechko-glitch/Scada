...
<?php
	...
	if(empty($_COOKIE[$Cook_Name])){
		...
		# Getting IMEI
		$Mysql_Query = "select * from user_master order by Firstname";
		...
		if($Mysql_Record_Count>=1){
			while($Fetch_Result = mysql_fetch_array($Mysql_Query_Result)){
				$ALLUser_Array[$Fetch_Result['Account_ID']] = $Fetch_Result['Username'];
				$ALLParent_Array[$Fetch_Result['Account_ID']] = $Fetch_Result['Parent_ID'];
				$FirstName[$Fetch_Result['Firstname']] = $Fetch_Result['Firstname'];
			}
		}
	...
	$Table_Name = "eb_data_closing_time";
	$Duplicate_Column = "";
	
	//Text boxes
	$Form_Fields[] = array('2','2','Username','Account_ID',$ALLUser_Array,'*','1','selectcls','','----- Select User-----','',$ALLParent_Array);

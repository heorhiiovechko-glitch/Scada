<?php
	...
	$Order_By = "Account_ID desc"; // Column name to be order and which order (asc or desc)
	$Records_Per_Page = 20;
	$Pagination_No_Count = 2;
	$Edit_ID_Column = "Account_ID"; // Unique value should be give for whatever query you do in that table
	...
	//$Form_Fields[] = array('1','3','Account','Account_ID',$Firstname_Admin_Array,'','1','','Filter_Sel2','onchange="Filter_Func(this.id,this.value,'.$Title_Head1.')"','----- Account Details -----',$_REQUEST['Account_ID']);
	...
	if( (isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])) || (isset($_REQUEST['SearchD'])) ){
		...
		if(isset($_REQUEST['search'])){
			...
			if($_REQUEST['search'] != '')
				$Ext_Query = " and";
			$Where_Query= "where (Parent_ID = ".$_REQUEST['P_ID']." or Account_ID = ".$_REQUEST['P_ID'].") $Ext_Query ".$Where_Query_Combind."";
			...
			<table border="0" cellpadding="0" cellspacing="0" width="985" height="70" align="left" class="List_Tab">
				...
				$P_ID1 = $_REQUEST['P_ID1'];
					if(isset($_REQUEST['P_ID'])){
						$P_ID = $_REQUEST['P_ID'];
						//$P_ID1 = $Cook_Variable[5];
				?>
                <tr>
                	<td align="center">
                        <table border="0" cellpadding="3" cellspacing="1" width="980" height="40px" align="left" class="info_tab">
                            <tr>
                                <td width="120px;" style="padding-left:10px" class="inner_td"><b>Account Name :</b> </td>
                                <td width="250px;" align="left" class="inner_td"><?=$Firstname_Lastname_Array[$P_ID];?> (<?=$Username_Array[$P_ID];?>)</td>
                                <td width="80px;" align="center" class="inner_td"><a href="List_Device.php?P_ID=<?=$P_ID?>&P_ID1=<?=$P_ID1?>">List Windmills</a></td>
								<td width="80px;" align="center" class="inner_td">&nbsp;</td>
...
?>			
				...
				echo $utid1;
					...
					if($Query_To_Mysql_Search)
						...
						$Select_Mysql = "SELECT $Edit_Column $Mysql_Select_Column FROM $Table_Name where Parent_ID = ".$_REQUEST['P_ID']." or Account_ID = ".$_REQUEST['P_ID']." order by $Order_By";
				...
				<tr bgcolor="#F0F0F0" height="25px">
					...
					<?
						...
						if($XLS == 0){
							echo '<td class="admin-menu-heading"><div align="left"><a href="#" title="User View"><strong>User View</strong></a></div></td>';
						}
					?>
			  	</tr>
				<tr>
...
echo $utid;
					...
					while($Fetch_Result = mysqli_fetch_array($Result)){
						...
						echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
							...
							//user view icon
							echo "<td style='font-size:12px;text-align:left'>";
							echo "<a href='../index.php?uname=".$Fetch_Result['Username']."&upass=".$Fetch_Result['Password']."&dologin=Login' title='User View' target='_New'><img src='./images/icon_alien.gif' height='20px' width='20px' title='User View'></a>&nbsp;&nbsp;&nbsp;&nbsp;";
							echo "</td>";
							echo "</tr>";

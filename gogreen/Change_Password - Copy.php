<?php
	...
	//Text boxes
	$Form_Fields[] = array('1','2','New Password','Password','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','2','Password Again','Password1','','*','E|N|0','','txtbox','','');

	//Submit Button
	$Form_Fields[] = array('1','5','',$Submit_Txt,'Change Password','','','','submit_but','');
	...
	if(isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])){
		...
		if($Record_Count == 0){
			...
			if($_REQUEST['Password']!= $_REQUEST['Password1']){
				$msg = "Both Passwords Should be match";
			}
			else{
				if($Query_To_Mysql_Join == '')
					$Query_To_Mysql = "update $Table_Name set Password = '".$_REQUEST['Password']."' where Account_ID = '".$Cook_Variable[3]."'";
				else
					...
					if($Query_To_Mysql_Result){
						...
						$Msg = urlencode("Password Record has been added successfully");
...
?>
		...
		<?
			if(isset($msg)){
		?>	
				<div style="padding-left:0px;"><span class="msg"><?=$msg?></span>
        <?
			}
		?>

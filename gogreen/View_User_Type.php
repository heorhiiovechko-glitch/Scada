<?php
	...
	if(empty($_COOKIE[$Cook_Name]) && $Cook_Variable[2] == 3){
	...
	$Mysql_Select_Column = array ('User_Type_ID','User_Type','User_Type_Description');
	// For Field Heading
	$Field_Name_Heading = array ('User Type ID','User Type','Description');
								...
								while(false != ($File_Con = readdir($Open_Dir))){
									if(($File_Con != ".") and ($File_Con != "..")){
										...
										$Check_Pos = strpos($File_Con, $View_ID."_".$Photo[0]."_")."I";
											...
											if( ($Check_Pos == '0I') ) {
												$File_View = '<a href="'.$Photo[1].'/'.$DirFiles_Array.'" target="_New" class="File_Con">'.$DirFiles_Array.'</a>';
												echo $Display_List[] = "<tr class='$Row_Cls' height='25'>";
												echo "<td class='Row_Td' width='200px'>".$Photo[0]."</td>";	
												echo "<td class='Row_Td'>".str_replace($View_ID.'_'.$Photo[0].'_','',$File_View)."</td>";
												echo "</tr>";	
											}
									}
								}

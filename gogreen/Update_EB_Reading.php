<?php
	...
	$Edit_Record = base64_decode($_REQUEST['Edit']);
	$Edit_Column = 'IMEI';
	$Sel_Date = date("Y-m-d",strtotime($_REQUEST['Date']));
	...
	$Table_Name = "EB_UPDATE_DATA";
	$Duplicate_Column = "";
	
	//Text boxes
	$Form_Fields[] = array('1','1','DGR EB Report Date','Date_Stamp',''.date("Y").'','*','E|N|0','onblur="GetEBData('.$Edit_Record.',this.value)"','txtbox','Cal','');
	$Form_Fields[] = array('1','1','Generation(kwh)','EB1','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','1','Import(kwh)','EB2','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','1','RKVAH Import(Units)','EB3','','*','E|N|0','','txtbox','','');
	$Form_Fields[] = array('1','9','Remarks','Remarks','','*','E|N|0','','txtarea','','');
	$Form_Fields[] = array('1','7','QType','QType','','*','E|N|0','','txtarea','','');
	
	//Submit Button
	$Form_Fields[] = array('1','5','',$Submit_Txt,'Update EB Reading','','','','submit_but','');
	...
	if(isset($_REQUEST[$Form_Fields[$Submit_Pos][3]])){
		...
		if($Record_Count == 0){
				if($_REQUEST['QType'] == '1'){
					echo $Query_To_Mysql = "Update EB_UPDATE_DATA set EB1 = '".$_REQUEST['EB1']."',EB2 = '".$_REQUEST['EB2']."',EB3 = '".$_REQUEST['EB3']."',Remarks = '".$_REQUEST['Remarks']."' where ".$Edit_Column." = '".$Edit_Record."' and Date_Stamp = '".$Sel_Date."'";
				}
				elseif($_REQUEST['QType'] == '0'){
					echo $Query_To_Mysql = "Insert into EB_UPDATE_DATA  (IMEI,EB1,EB2,EB3,Remarks, Date_Stamp) values ('".$Edit_Record."','".$_REQUEST['EB1']."','".$_REQUEST['EB2']."','".$_REQUEST['EB3']."','".addslashes($_REQUEST['Remarks'])."','$Sel_Date')";
				}
				//echo $Query_To_Mysql = "Update $Table_Name set ".substr($Query_For_Update,0,-1)." where ".$Edit_Column." = '".$Edit_Record."' and Date_Stamp = '".$Sel_Date."'";
				...
				if($Query_To_Mysql_Result){
					...
					header("Location:List_DGR_EB_Report.php?msg=$Msg");
					exit;
				}
		}
	}
...
<?php
		...
		$Select_Query = "select * from $Table_Name where $Edit_Column = '$Edit_Record' and Date_Stamp = '".$Sel_Date."'";
					...
					<div id="admin_left_inner">
                           ...
                           <?
									...
									elseif($Forms[1] == 4){
										...
										if($Forms[3] == 'QType')
											$Get_Record[$Forms[3]] = $Select_Count;
                            ?>
										<?=Func_Forms_Element($Forms[0],$Forms[1],$Forms[3],$Get_Record[$Forms[3]],$Forms[8],$Forms[7]);?>
                                        <?
											if($Forms[3] == 'Password')

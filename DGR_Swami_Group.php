...
?>
		<tr>
			...
			<?php  }if($FType==7 || $FType==8){?>
				<a href='channel8_swami_ajax.php?<?=$_SERVER['QUERY_STRING']?>&XLS=1' style='text-decoration:underline;font-weight:bold;'>Click here</a>
			<?php  }if($FType==10){?>
...
?>					
        ...
        <tr>
            <td width="850px">
<table width="950px" border="<?=$XLS == 1?'1':'0'?>" align="left" cellpadding="1" cellspacing="1" class="innertab1_">	
	<?php
 ...
 <?php 
	...
	<?php
		...
		if($Cook_Variable[2] ==3 || $Cook_Variable[2] ==2)
			...
			$Device_Query="select Device_Name,Format_Type,hour(Closing_Time) as Closing_Time, Connect_Feeder,Site_Location,State,IMEI,Db_Name from device_register where Parent_ID=" .$Cook_Variable[6] ."  order by Device_Order";
		elseif($Cook_Variable[2] ==4)
			$Device_Query="select Device_Name,Format_Type,hour(Closing_Time) as Closing_Time, Connect_Feeder,Site_Location,State,IMEI,Db_Name from device_register where Account_ID='".$Account_ID."'  order by Device_Order";
		//echo $Device_Query;
...
//print_r($GD_Hours);
	...
	$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F1_IMEI).")  and (Date_S= '".$Date_Stamp."')";
	if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Run_Max']-$Fetch_Result['Run_Min'];
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Run[$Fetch_Result['IMEI']][$DATE_Val]<'100'?'24':$Run[$Fetch_Result['IMEI']][$DATE_Val];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1H_Max']-$Fetch_Result['Gen1H_Min'];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'100'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]-$Gen1[$Fetch_Result['IMEI']][$DATE_Val];
								if($Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]==(-1))
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=0;
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val];
								$Array_Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val] = 24-($Fetch_Result['Line_Max']-$Fetch_Result['Line_Min']);
								$Array_GD[$Fetch_Result['IMEI']][$DATE_Val]=$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=((24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]) / 24) * 100 ;
								$Array_Lull[$Fetch_Result['IMEI']][$DATE_Val]=$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Loss_Due_To_GD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val];
								$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]=24-($GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]+$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]+$Gen1[$Fetch_Result['IMEI']][$DATE_Val]);							
								$Loss_Due_To_BD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]; 
								$MA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=(((24-$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])-($BD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) / (24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) *100;
									}//end while
								}
				...
				$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F2_IMEI).")  and (Date_S= '".$Date_Stamp."')";
		...
		if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<'500'?'24':$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val];
...
$GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val]=($GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] >=0 && $GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] <=24)?$GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] : '0';
		...
		$Array_BD[$IMEI_Val][$DATE_Val]=$BD_Hours[$IMEI_Val][$DATE_Val]>0 && $BD_Hours[$IMEI_Val][$DATE_Val]<=25?$BD_Hours[$IMEI_Val][$DATE_Val]:'0';								
				...
				$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F3_IMEI).") and (Date_S= '".$Date_Stamp."')";								//echo $Gen_Mysql_Query;
				if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            }
	...
	$POC_Mysql_Query = "select IMEI,Date_S,Error_Type,Time_Diff,sum(Time_Diff) as Diff from $Cook_Variable[7].pocket_time_calc where IMEI in (".implode(",",$F3_IMEI).")  and (Date_S= '".$Date_St."' OR  Date_S='". $Yester_Stamp ."')   and (case when (Date_S='$Date_St') then  hour((cast(Time_S as time)))>='$Closing_Time[$IMEI_Val]' else hour(cast(Time_S as time))<'$Closing_Time[$IMEI_Val]' end) group by IMEI,Error_Type";
//echo $POC_Mysql_Query;
		if (!$POC_Mysql_Query_Result = $db->query($POC_Mysql_Query))
            {
                die($db->error);
...
$Array_BD[$IMEI_Val][$DATE_Val]=$BD_Hours[$IMEI_Val][$DATE_Val]>0 && $BD_Hours[$IMEI_Val][$DATE_Val]<=25?$BD_Hours[$IMEI_Val][$DATE_Val]:'0';								
								...
								$Loss_Due_To_GD[$IMEI_Val][$DATE_Val] = ($Total_Gen[$IMEI_Val][$DATE_Val]/$Run_Hours[$IMEI_Val][$DATE_Val]) * $GD_Hours[$IMEI_Val][$DATE_Val];
				...
				$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F4_IMEI).")  and (Date_S= '".$Date_Stamp."')";
				//echo $Gen_Mysql_Query;
								if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
...
$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F6_IMEI).")  and (Date_S= '".$Date_Stamp."')";
	if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            }
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Array_Gen[$Fetch_Result['IMEI']][$DATE_Val]=$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]<=15000?$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Run_Max']-$Fetch_Result['Run_Min'];
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Run[$Fetch_Result['IMEI']][$DATE_Val]<'500'?'24':$Run[$Fetch_Result['IMEI']][$DATE_Val];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1H_Max']-$Fetch_Result['Gen1H_Min'];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'50'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]-$Gen1[$Fetch_Result['IMEI']][$DATE_Val];
								if($Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]==(-1))
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=0;
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val];
								...
								$Loss_Due_To_BD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]; 
								$MA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=(((24-$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])-($BD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) / (24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) *100;
								$Array_BD[$Fetch_Result['IMEI']][$DATE_Val]=$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';								
									}//end while
								}
							}//endif isset
							...
							if($Format[$IMEI_Val]== 7){
								...
								$Gen_Mysql_Query="Select IMEI,Date,Windspeed,Min(Active_Total_Gen_Export) as Gen1_Min, Active_Total_Gen_Export as Gen1_Max, Min(Min3_Active_Power) as Run_Min, Max(Min3_Active_Power) as Run_Max, Min(Min3_Wind_Dir) as Gen1H_Min, Max(Min3_Wind_Dir) as Gen1H_Max, Min(Min3_Wind_Speed) as Line_Min, Max(Min3_Wind_Speed) as Line_Max, Min(Stopped_Hours) as Import_Min, Max(Stopped_Hours) as Import_Max, Min(Grid_failure_Hours) as Total_hrs_Min, Max(Grid_failure_Hours) as Total_hrs_Max  from va_swami.device_data_f7 Where IMEI ='".$IMEI_Val."' and   (Date_S= '".$Date_Stamp."')  and Active_Total_Gen_Export!='' and Active_Total_Gen_Export>'0' and Min3_Active_Power>'0' group by IMEI order by Date_S desc,Record_Index desc ";
								echo $Gen_Mysql_Query;
if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Import_Max']-$Fetch_Result['Import_Min'];
								$Array_Import[$Fetch_Result['IMEI']][$DATE_Val]=$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1_Max']*1000;
								$Array_Gen[$Fetch_Result['IMEI']][$DATE_Val]=$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]<=18000?$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Run_Max']-$Fetch_Result['Run_Min'];
								$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1H_Max']-$Fetch_Result['Gen1H_Min'];//no servicehrs
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'50'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Array_Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val] = $Fetch_Result['Line_Max']-$Fetch_Result['Line_Min'];
								$Array_BD[$Fetch_Result['IMEI']][$DATE_Val]=$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';								
								$Array_GD[$Fetch_Result['IMEI']][$DATE_Val]=$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=((24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]) / 24) * 100 ;
								$Loss_Due_To_GD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val];
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=24-($GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]+$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]+$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]);
								$Array_Lull[$Fetch_Result['IMEI']][$DATE_Val]=$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Loss_Due_To_BD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]; 
								$MA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=(((24-$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])-($BD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) / (24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val])) *100;
							...
							if($Format[$IMEI_Val]== 8){
								...
								$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F8_IMEI).")  and (Date_S= '".$Date_Stamp."')";
if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            }
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Windspeed[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['WindSpeed'];
								$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Import_Max'];
								$Array_Import[$Fetch_Result['IMEI']][$DATE_Val]=$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]<=500?$Import_LCS[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1_Max'];
								$Array_Gen[$Fetch_Result['IMEI']][$DATE_Val]=$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]<=15000?$Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Run_Max'];
								$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1H_Max'];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'50'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Array_Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val] = 24-($Fetch_Result['Line_Max']);
								$Array_BD[$Fetch_Result['IMEI']][$DATE_Val]=$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';								
								$Array_GD[$Fetch_Result['IMEI']][$DATE_Val]=$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=((24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]) / 24) * 100 ;
								$Loss_Due_To_GD[$Fetch_Result['IMEI']][$DATE_Val] = ($Total_Gen[$Fetch_Result['IMEI']][$DATE_Val]/$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]) * $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val];
...
$Gen_Mysql_Query="select IMEI,Date_S,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F10_IMEI).")  and (Date_S= '".$Date_Stamp."')";
					//echo $Gen_Mysql_Query;
if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Run_Max']-$Fetch_Result['Run_Min'];
								$Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Run[$Fetch_Result['IMEI']][$DATE_Val]<'100'?'24':$Run[$Fetch_Result['IMEI']][$DATE_Val];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=($Fetch_Result['Gen1H_Max']-$Fetch_Result['Gen1H_Min'])+($Fetch_Result['Gen2H_Max']-$Fetch_Result['Gen2H_Min']);
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'50'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Run[$Fetch_Result['IMEI']][$DATE_Val]-$Gen1[$Fetch_Result['IMEI']][$DATE_Val];
								if($Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]==(-1))
								$Lull_Hours[$Fetch_Result['IMEI']][$DATE_Val]=0;
								$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val];
								$Array_Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val] = 24-($Fetch_Result['Line_Max']-$Fetch_Result['Line_Min']);
								$Array_GD[$Fetch_Result['IMEI']][$DATE_Val]=$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GA_Percent[$Fetch_Result['IMEI']][$DATE_Val]=((24 - $GD_Hours[$Fetch_Result['IMEI']][$DATE_Val]) / 24) * 100 ;
				...
				<td class="tab-head-td1" align="left"><?=($Total_Gen[$IMEI_Val][$DATE_Val] >=0 && $Total_Gen[$IMEI_Val][$DATE_Val] <=(150000*($diff+1)))?round($Total_Gen[$IMEI_Val][$DATE_Val],2): '000'?></td>                  
				...
				<td class="tab-head-td1" align="left"><?=($Import_LCS[$IMEI_Val][$DATE_Val]!= '' && $Import_LCS[$IMEI_Val][$DATE_Val] >=0 || $Import_LCS[$IMEI_Val][$DATE_Val] ==0)?round($Import_LCS[$IMEI_Val][$DATE_Val],2) : '000'?></td>                 
                     	...
                     	<td class="tab-head-td1" align="left"><?=($Run_Hours[$IMEI_Val][$DATE_Val] >=0 && $Run_Hours[$IMEI_Val][$DATE_Val] <=24)?round($Run_Hours[$IMEI_Val][$DATE_Val],2) : '000'?></td>                               
                        <td class="tab-head-td1" align="left"><?=($GD_Hours[$IMEI_Val][$DATE_Val] >=0 && $GD_Hours[$IMEI_Val][$DATE_Val] <=24)?round($GD_Hours[$IMEI_Val][$DATE_Val],2) : '000'?></td> 
			<td class="tab-head-td1" align="left"><?=($BD_Hours[$IMEI_Val][$DATE_Val] >=0 && $BD_Hours[$IMEI_Val][$DATE_Val] <=24)?round($BD_Hours[$IMEI_Val][$DATE_Val],2) : '000'?></td>                                     
                       
			<!--<td class="tab-head-td1" align="left"><?=$MA_Percent[$IMEI_Val][$DATE_Val] != ''?round($MA_Percent[$IMEI_Val][$DATE_Val],2) : '0'?></td>   
			<td class="tab-head-td1" align="left"><?=$BD_Hours[$IMEI_Val][$DATE_Val] != ''?$BD_Hours[$IMEI_Val][$DATE_Val] : '0'?></td>    
			<td class="tab-head-td1" align="left"><?=$Loss_Due_To_GD[$IMEI_Val][$DATE_Val] != ''?round($Loss_Due_To_GD[$IMEI_Val][$DATE_Val],2) : '0'?></td> 
			<td class="tab-head-td1" align="left"><?=$Loss_Due_To_BD[$IMEI_Val][$DATE_Val] != ''?round($Loss_Due_To_BD[$IMEI_Val][$DATE_Val] ,2): '0'?></td>     
                        <td class="tab-head-td1" align="left"><?=$Remarks[$IMEI_Val][$DATE_Val] != ''?$Remarks[$IMEI_Val][$DATE_Val] : 'Nil'?></td> -->

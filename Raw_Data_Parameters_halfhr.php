...
error_reporting(0);
	...
	$Raw_Data_Query="select sec_to_time(time_to_sec(Time_S)- time_to_sec(Time_S)%(30*60)) AS Time_diff,GRPM,RRPM,WindSpeed,Status,Pitch,Power,Ambient_Temp,Hydraulic_Temp,Gear_Temp,Gen1_Temp,Nacel_Temp,Control_Temp,Bearing_Temp,PAT_Gen0,PAT_Gen1,PAT_Gen2,RPhase_Volt,YPhase_Volt,BPhase_Volt,RPhase_Current,YPhase_Current,BPhase_Current,Power_Factor,Date_S,Time_S from $Cook_Variable[7].$Table_Name where  IMEI = '".$IMEI."' and (Date_S >= '".$From_YMD."' and  Date_S <= '".$To_YMD."') and Status!='' and ID_Number!=''  group by Date_S,Time_diff order by Record_Index desc ";

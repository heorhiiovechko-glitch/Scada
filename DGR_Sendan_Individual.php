...
?>
		...
		<tr>
			...
			<?php }if($FType==7 || $FType==8){?>
				...
				<a href='channel8_Sendan_ajax.php?<?=$_SERVER['QUERY_STRING']?>&XLS=1' style='text-decoration:underline;font-weight:bold;'>Click here</a>
...
//echo $Gen_Mysql_Query;
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
				...
				while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$Array_Total_hrs[$DATE_Val] =$Total_hrs[$DATE_Val]>0 ?$Total_hrs[$DATE_Val]:'0';
...
if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                ...
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
								...
								$GD_Hours[$DATE_Val] = 24-($Fetch_Result['Line_Max']-$Fetch_Result['Line_Min']);
								$Array_GD[$DATE_Val]=$GD_Hours[$DATE_Val];
								$GA_Percent[$DATE_Val]=((24 - $GD_Hours[$DATE_Val]) / 24) * 100 ;
								$Array_Lull[$DATE_Val]=$Lull_Hours[$DATE_Val];
								$Loss_Due_To_GD[$DATE_Val] = ($Total_Gen[$DATE_Val]/$Run_Hours[$DATE_Val]) * $GD_Hours[$DATE_Val];
								$BD_Hours[$DATE_Val]=24-($GD_Hours[$DATE_Val]+$Lull_Hours[$DATE_Val]+$Gen1[$DATE_Val]);							
								$Loss_Due_To_BD[$DATE_Val] = ($Total_Gen[$DATE_Val]/$Run_Hours[$DATE_Val]) * $BD_Hours[$DATE_Val]; 
								//$MA_Percent[$DATE_Val]=(((24-$GD_Hours[$DATE_Val])-($BD_Hours[$DATE_Val])) / (24 - $GD_Hours[$DATE_Val])) *100;
								$MA_Percent[$DATE_Val]=((24-$BD_Hours[$DATE_Val]) / 24 ) *100;
								$Array_BD[$DATE_Val]=$BD_Hours[$DATE_Val];
				}
			}
				...
				<td class="tab-head-td1" align="left"><?=round($Total_Gen[$DATE_Val],2)?> </td>                  
				
              	<td class="tab-head-td1" align="left"><?= round($Array_Total_hrs[$DATE_Val],2) ?></td>

                <td class="tab-head-td1" align="left"><?=round($Array_Run[$DATE_Val],2) ?></td>                               
                
				<td class="tab-head-td1" align="left"><?=round($Array_Line[$DATE_Val],2) ?></td> 
				
				<td class="tab-head-td1" align="left"><?=round($Array_Import[$DATE_Val],2) ?></td> 
						...
						?>
							...
							<td class="tab-head-td1" align="left"><b><?=(arraySumRecursive($Array_Gen)>=0)? arraySumRecursive($Array_Gen):'000' ?></b></td>
							<td class="tab-head-td1" align="left"><b><?=(arraySumRecursive($Array_Total_hrs)>=0)? arraySumRecursive($Array_Total_hrs):'000'?></b></td>
							<td class="tab-head-td1" align="left"><b><?=(arraySumRecursive($Array_Run)>=0) ? arraySumRecursive($Array_Run)>=0:'000'?></b></td>
							<td class="tab-head-td1" align="left"><b><?=(arraySumRecursive($Array_Line)>=0) ? arraySumRecursive($Array_Line)>=0:'000'?></b></td>

 ...
 <?php 
	...
	<?php
		...
		if($Cook_Variable[2] ==3 || $Cook_Variable[2] ==2)
			...
			$Device_Query="select Device_Name,Format_Type,hour(Closing_Time) as Closing_Time, Connect_Feeder,Site_Location,State,IMEI from device_register where Parent_ID=" .$Cook_Variable[6] ."  and Format_Type='6' order by Connect_Feeder DESC";
		elseif($Cook_Variable[2] ==4)
			$Device_Query="select Device_Name,Format_Type,hour(Closing_Time) as Closing_Time, Connect_Feeder,Site_Location,State,IMEI from device_register where Account_ID=" .$Cook_Variable[3] ."  and Format_Type='6' order by Connect_Feeder DESC";
		//echo $Device_Query;
		...
		if($Device_Query_Result_Count>=1){
			while($Fetch_Result = mysql_fetch_array($Device_Query_Result )){
				...
				$Format[$Fetch_Result['Format_Type']] = $Fetch_Result['IMEI'];
...
foreach($DGR_IMEI as $IMEI_Val){
	...
	$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F1_IMEI).")  and (Date_S= '".$Date_Stamp."')";
...
$GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val]=($GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] >=0 && $GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] <=24)?$GD_Hours[$POC_Fetch_Result['IMEI']][$DATE_Val] : '0';
		...
		$Array_BD[$IMEI_Val][$DATE_Val]=$BD_Hours[$IMEI_Val][$DATE_Val]>0 && $BD_Hours[$IMEI_Val][$DATE_Val]<=25?$BD_Hours[$IMEI_Val][$DATE_Val]:'0';								
				...
				$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F3_IMEI).") and (Date_S= '".$Date_Stamp."')";								//echo $Gen_Mysql_Query;
...
$Array_BD[$IMEI_Val][$DATE_Val]=$BD_Hours[$IMEI_Val][$DATE_Val]>0 && $BD_Hours[$IMEI_Val][$DATE_Val]<=25?$BD_Hours[$IMEI_Val][$DATE_Val]:'0';								
				...
				$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Gen2_Min,Gen2_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F4_IMEI).")  and (Date_S= '".$Date_Stamp."')";
				//echo $Gen_Mysql_Query;
								$Gen_Mysql_Query_Result = mysql_query($Gen_Mysql_Query) or die(mysql_error());   
...
$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F6_IMEI).")  and (Date_S= '".$Date_Stamp."')";
...
$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Gen2H_Min,Gen2H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F10_IMEI).")  and (Date_S= '".$Date_Stamp."')";
					//echo $Gen_Mysql_Query;
...
?>
			 ...
			 <td class="tab-head-td1" align="left"><?=($Windspeed[$IMEI_Val][$DATE_Val] >=0)?round($Windspeed[$IMEI_Val][$DATE_Val],2) : '000'?></td>  	

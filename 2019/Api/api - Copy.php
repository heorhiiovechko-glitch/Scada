...
function get_detail(){
	...
	$ER_Mysql_Query = "select Status as Log,Date_S,Time_S from $Database_Name.$Table_Name where IMEI='".$IMEI_Decode."' and Status !='' order by Record_Index desc limit 1";
	...
	if($Mysql_Query_Result->num_rows >= 1){
		...
		$data['GAD_for_Yesterday'] = ($GAD_Yesterday > 15000 || $GAD_Yesterday < 0) ? "Nil":$GAD_Yesterday." Kwh";
		...
		$data['GAD_for_This_month'] = ($GAD_Thismonth > 300000 || $GAD_Thismonth < 0) ? "Nil":$GAD_Thismonth." Kwh";
	}
...
//echo $Status;
			...
			$single['G4_Temp'] = $G4_Temp;								
			$single['HTSCno'] = $HTSCno;								
			$single['G6_Temp'] = $G6_Temp;								
			$single['G3'] = $G3;								

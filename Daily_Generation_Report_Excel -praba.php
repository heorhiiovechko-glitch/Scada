            ...
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
							...
							if($Format[$IMEI_Val]== 7){
								...
								$Gen_Mysql_Query="select IMEI,Date_S,Windspeed,Gen1_Min,Gen1_Max,Run_Min,Run_Max,Gen1H_Min,Gen1H_Max,Line_Min,Line_Max,abs(Import_Min) as Import_Min,abs(Import_Max) as Import_Max from daily_data where IMEI in (".implode(",",$F7_IMEI).")  and (Date_S= '".$Date_Stamp."')";
if (!$Gen_Mysql_Query_Result = $db->query($Gen_Mysql_Query))
            {
                die($db->error);
            }
            if($Gen_Mysql_Query_Result->num_rows >= 1)
            {
                while($Fetch_Result = $Gen_Mysql_Query_Result->fetch_array()) {
							...
							if($Dbname[$IMEI_Val]!='va_aalayam') {
								...
								$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]=$Fetch_Result['Gen1H_Max']-$Fetch_Result['Gen1H_Min'];
								$Gen1[$Fetch_Result['IMEI']][$DATE_Val]=$Gen1[$Fetch_Result['IMEI']][$DATE_Val]>'24' && $Gen1[$Fetch_Result['IMEI']][$DATE_Val]<'50'?'24':$Gen1[$Fetch_Result['IMEI']][$DATE_Val];	
								$Array_Run[$Fetch_Result['IMEI']][$DATE_Val]=$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$Run_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';
								$GD_Hours[$Fetch_Result['IMEI']][$DATE_Val] = 24-($Fetch_Result['Line_Max']-$Fetch_Result['Line_Min']);
								$Array_BD[$Fetch_Result['IMEI']][$DATE_Val]=$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]>0 && $BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]<=25?$BD_Hours[$Fetch_Result['IMEI']][$DATE_Val]:'0';								
...
?>
			...
			<td class="tab-head-td1" align="left"><?=($BD_Hours[$IMEI_Val][$DATE_Val] >=0 && $BD_Hours[$IMEI_Val][$DATE_Val] <=24)?round($BD_Hours[$IMEI_Val][$DATE_Val],2) : '000'?></td>                                     
                        <td class="tab-head-td1" align="left"><?=($Lull_Hours[$IMEI_Val][$DATE_Val] >=0 && $Lull_Hours[$IMEI_Val][$DATE_Val] <=24)?round($Lull_Hours[$IMEI_Val][$DATE_Val],2) : '000'?></td>   
                        <td class="tab-head-td1" align="left"><?=$GA_Percent[$IMEI_Val][$DATE_Val] != ''?round($GA_Percent[$IMEI_Val][$DATE_Val],2) : '000'?></td>
			<?php
			if($Cook_Variable[7]='va_mtk') {
				?>
						<td class="tab-head-td1" align="left"><?=($Loss_Due_To_GD[$IMEI_Val][$DATE_Val] >0 && $Loss_Due_To_GD[$IMEI_Val][$DATE_Val] <=10000)?round($Loss_Due_To_GD[$IMEI_Val][$DATE_Val],2) : '000'?></td>
						<td class="tab-head-td1" align="left"><?=($Loss_Due_To_BD[$IMEI_Val][$DATE_Val] >0 && $Loss_Due_To_BD[$IMEI_Val][$DATE_Val] <=10000)?round($Loss_Due_To_BD[$IMEI_Val][$DATE_Val],2) : '000'?></td>
						<?php
			}
			?>

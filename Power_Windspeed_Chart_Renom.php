...
//Getting All Hours Calculation
		...
		$Mysql_Query="select @a:=@a+1 serial_number,Date_S, TIME(from_unixtime(ROUND(unix_timestamp(TIME_S) / (60*10)) * 60 * 10)) as TenMinutes,MAX(GREATEST(Active_Total_Gen_Import,0)) as GEN_Max ,MIN(GREATEST(Active_Total_Gen_Import,0)) as GEN_Min ,ROUND(AVG(GREATEST(POWER,0)),2) as Power, ROUND(AVG(WINDSPEED),2) as WindSpeed, Date_S,Time_S  from $Cook_Variable[7].device_data_f7 , (SELECT @a:= 0) AS a where IMEI = '".$IMEI."' and /*Date_S='$Extract_Date'*/ (Date_S >= '".$Extract_Date."' and  Date_S <= '".$Extract_Date_Till."') and Active_Total_Gen_Import between 0 and 20000000  group by Date_S,TenMinutes";
		
		/*elseif($Format_Type==11)
		$Mysql_Query="select @a:=@a+1 serial_number,Date_S, TIME(from_unixtime(ROUND(unix_timestamp(TIME_S) / (60*10)) * 60 * 10)) as TenMinutes,MAX(GREATEST(Bridge2_dcc,0)) as GEN_Max ,MIN(GREATEST(Bridge2_dcc,0)) as GEN_Min ,ROUND(AVG(GREATEST(Bridge2_dcc,0)),2) as Power, ROUND(AVG(Bridge1_dcv),2) as WindSpeed, Date_S,Time_S  from $Cook_Variable[7].device_data_f11 , (SELECT @a:= 0) AS a where IMEI = '".$IMEI."' and  
		(Date_S >= '".$Extract_Date."' and  Date_S <= '".$Extract_Date_Till."') and Bridge2_dcc between 0 and 20000000  group by Date_S,TenMinutes";*/
		
		
		elseif($Format_Type==11)
		$Mysql_Query = "
SELECT
    ROUND(Bridge1_dcv*2)/2 AS WindSpeed,
    ROUND(AVG(Bridge2_dcc),2) AS AvgPower,
    MAX(Bridge2_dcc) AS MaxPower,
    MIN(Bridge2_dcc) AS MinPower,
    COUNT(*) AS Samples
FROM
    $Cook_Variable[7].device_data_f11
WHERE
    IMEI='".$IMEI."'
    AND Date_S BETWEEN '".$Extract_Date."' AND '".$Extract_Date_Till."'
    AND Bridge1_dcv > 0
    AND Bridge2_dcc >= 0
    AND Bridge2_dcc <= 850
GROUP BY
    ROUND(Bridge1_dcv*2)/2
ORDER BY
    WindSpeed";
		...
		if (!$Mysql_Query_Result = $db->query($Mysql_Query))
			...
			while($row = $Mysql_Query_Result->fetch_assoc())
{
    $ChartWind[]  = $row['WindSpeed'];   // Bridge1_dcv
    $ChartPower[] = $row['Power'];       // Bridge2_dcc
}
echo "<h3>Wind Speed</h3>";
foreach($ChartWind as $value)
{
    echo $value . "<br>";
}

echo "<h3>Power</h3>";
foreach($ChartPower as $value)
{
    echo $value . "<br>";
}
		...
		if($Format_Type==10 || $Format_Type==11 || $Format_Type==7 || $Format_Type==9  ){
			$GEN_Min_Value=0;
...
elseif($Format_Type==11)
$Filename="channel11_type11.php?c1=".$_REQUEST['c1']."&l=".$_REQUEST['l']."&FType=".$Format_Type;
...
<script>
  google.charts.load('current', {packages: ['corechart']});  
    ...
    title: 'Power Curve Analysis (<?=date("d-m-Y",strtotime($Extract_Date))?> to <?=date("d-m-Y",strtotime($Extract_Date_Till))?>)',
    titleTextStyle: {
        fontSize: 18,
        bold: true,
        color: '#222',
    ...
    series: {
        0: { color: '#388e3c', pointSize: 7 },  // Measured
        1: { color: '#e53935', lineWidth: 3 }    // Reference
    },
 ...
 <script type="text/javascript">
      ...
      function drawChart() {
		   ...
		   $refArray = array(0,0,0,7.60,27.70,72.60,136.90,225.40,343.80,489.80,646.20,742.60,798.30,823.30,840.60,846.40,848.70,850.0,850.0,850.0,850.0,850.0);
...
function export_PDF() {
    ...
    const device = document.getElementById('Owner_Name').value + " - " +
    ...
    html2canvas(chart, { scale: 2 }).then(canvas => {
        ...
        pdf.setFontSize(16);
        pdf.text(device, 15, 20);

        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(12);
        pdf.text("Power and Windspeed Graph", 15, 30);

        pdf.addImage(imgData, 'JPEG', 10, 40, 190, 130);

        pdf.save("Power_Wind_Graph.pdf");
    });
}
	  ...
	  <div id="expotPdfDiv" >
			<div id="output">
						...
						<table border="0" cellpadding="0" cellspacing="0" width="100%" hieght="auto">
							<tr>
								<td align=right>
									<input type=hidden name="Device_Name" id="Device_Name" value="<?= $Device_Name?>"><input type=hidden name="Owner_Name" id="Owner_Name" value="<?= $Firstname.' '.$Lastname?>"><button onclick="export_PDF()">Download PDF</button>
								</td>
							<tr>
								<td>
									<?php
